...
interface ManageUnlockFeesClientProps {
...
export default function ManageUnlockFeesClient({ userRole, userId, dbUserId }: ManageUnlockFeesClientProps) {
  const [fees, setFees] = useState<UnlockFeeData[]>([]);
  const [allFees, setAllFees] = useState<UnlockFeeData[]>([]);
  ...
  const [viewMode, setViewMode] = useState<"all" | "my-content">(userRole === "super_admin" ? "all" : "my-content");
  ...
  const fetchFees = useCallback(async () => {
    try {
      ...
      // Fetch unlock fees - for admins, filter by their content
      const filterParam = viewMode === "my-content" ? "?filter=my-content" : "";
      const response = await fetch(`/api/admin/unlock-fees${filterParam}`);
      if (!response.ok) {
        throw new Error("Failed to fetch unlock fees");
      }

      const data = await response.json();
      const fetchedFees = data.fees || [];
      
      if (viewMode === "my-content") {
        // Filter to only show fees for content owned by current user
        const myFees = fetchedFees.filter((fee: UnlockFeeData) => {
          const ownerId = fee.resource?.ownerId || fee.topic?.ownerId || fee.subject?.ownerId;
          return ownerId === dbUserId;
        });
        setFees(myFees);
      } else {
  ...
  const getContentOwner = (fee: UnlockFeeData) => {
    return fee.resource?.ownerId || fee.topic?.ownerId || fee.subject?.ownerId;
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex gap-2">
          {isSuperAdmin && (
            <>
              ...
              <Button
                variant={viewMode === "my-content" ? "default" : "outline"}
                onClick={() => setViewMode("my-content")}
              >
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {viewMode === "my-content" ? "My Content Unlock Fees" : "Content Unlock Fees"}
          </CardTitle>
          <CardDescription>
            {viewMode === "my-content"
              ? "Manage unlock fees for resources, topics, and subjects you've created. Users will pay these amounts via M-Pesa to unlock your content."
        ...
        <CardContent className="space-y-4">
          <div className="flex gap-4">
            <div className="flex-1 relative">
              ...
              <Input
                placeholder={viewMode === "my-content" ? "Search your content..." : "Search content..."}
                value={searchQuery}
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    {viewMode === "my-content" && (
                      <p className="text-sm mt-2">
                        Create some content first, then unlock fees will be automatically initialized
                      </p>
                    )}
                  </div>
                ) : (
